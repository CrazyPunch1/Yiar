...
public class GunScript : MonoBehaviour {
    ...
    public float attackRange = 2.0f;  // Range for melee attacks
    public int damageAmount = 10;     // Amount of health drained on hit
    public int damageAmount1 = 100;     // Amount of health drained on hit
    public LayerMask enemyLayer;      // Layer of enemies for raycast
    ...
    // Method to check if the ammo is full
    public bool IsAmmoFull()
    {
        return bulletsInTheGun >= amountOfBulletsPerLoad;
    }

    // Method to add ammo to the gun
    public void AddAmmo(int amount)
    {
        // Calculate the remaining space in the gun
        float spaceInGun = amountOfBulletsPerLoad - bulletsInTheGun;
        // Add only as many bullets as can fit in the magazine
        float bulletsToAdd = Mathf.Min(spaceInGun, amount);

        bulletsInTheGun += bulletsToAdd;

        // If we still have leftover ammo from the pickup, add it to bulletsIHave
        if (amount > bulletsToAdd)
        {
			bulletsIHave += (amount - bulletsToAdd);
        }
	...
	*/
	...
	[HideInInspector]
	public bool aiming;
    // Check if melee or aiming animations are playing
    ...
    // Detect input to trigger the melee attack coroutine
    void MeeleAttack()
    {
    ...
    // Coroutine to handle the melee animation and attack logic
    IEnumerator AnimationMeeleAttack()
    {
        handsAnimator.SetBool("meeleAttack", true);

        // Slight delay to sync with animation
        yield return new WaitForSeconds(0.1f);

        // Perform melee attack using raycast
        PerformMeeleAttack();

        handsAnimator.SetBool("meeleAttack", false);
    }

    // Function to perform melee attack using a raycast
    void PerformMeeleAttack()
    {
        RaycastHit hit;

        // Raycast from the player in the forward direction to detect enemies
        if (Physics.Raycast(transform.position, transform.forward, out hit, attackRange, enemyLayer))
        {
            // Check if the object hit has a Health component
            EnemyAiTutorial enemyHealth = hit.collider.GetComponent<EnemyAiTutorial>();
            if (enemyHealth != null)
            {
                enemyHealth.TakeDamage(damageAmount1);
            }

			 // Check if the object hit has a Health component
            EnemyAiTutorial1 enemyHealth1 = hit.collider.GetComponent<EnemyAiTutorial1>();
            if (enemyHealth1 != null)
            {
                enemyHealth1.TakeDamage(damageAmount1);
            }

			 // Check if the object hit has a Health component
            EnemyMeleeAI enemyHealth2 = hit.collider.GetComponent<EnemyMeleeAI>();
            if (enemyHealth2 != null)
            {
                enemyHealth2.TakeDamage(damageAmount1);
            }
        }
    }
